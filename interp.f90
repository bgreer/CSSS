...
Contains
  ...
  Subroutine Read_Grid()
    ...
    ! Get the grid
	 If (ash_grid) Then
	    ...
	    Print*, 'Have read old_nr=, nth=, nphi=', onr, nth, nphi
	 ...
	 Else
		Write(6,*)'Reading Rayleigh Grid'
		! Rayleigh Grid
	   open(unit=grid_unit,file=Trim(grid_filename),form='unformatted', status='old')
	   Read(grid_unit)onr
		Read(grid_unit)nth
		Read(grid_unit)nphi
		ntot = onr + nth + 5
		Allocate(oldr(onr), oldtheta(nth), phi(nphi), theta(nth), radius(nr))		
	   Read(grid_unit)(oldr(i),i=1,onr)
		Read(grid_unit)(oldtheta(i),i=1,nth)
	   Close(grid_unit)	
		Write(6,*)'nr,nth,nphi: ', onr, nth, nphi
		Write(6,*)'theta: ', oldtheta
		Write(6,*)'====================='
		Write(6,*)'radius: ', oldr
	 Endif
    ...
    phi = dphi*(/(Dble(it),it=0,nphi-1)/) - dpi
    theta = dth*(/(Dble(it),it=0,nth-1)/)+0.5d0*dth
    radius = rmax - dr*(/(Dble(ir),ir=0,nr-1)/)

    If (cartesian) Then
       Allocate(olddata(nphi,nth,onr),data(ncube,ncube,ncube),newx(ncube),newy(ncube),newz(ncube))
       newx = (2d0*rmax/Dble(ncube-1))*(/(Dble(it),it=0,ncube-1)/)-rmax
       newy = (2d0*rmax/Dble(ncube-1))*(/(Dble(it),it=0,ncube-1)/)-rmax
       newz = (2d0*rmax/Dble(ncube-1))*(/(Dble(it),it=0,ncube-1)/)-rmax
    Else
  ...
  Subroutine Interpolate()
    ...
    If (nfloat .eq. 1) Then
       ...
       !Read in bulk data
       Open(interp_unit, file=Trim(iter_filename), form='unformatted', access='direct', status='unknown',recl=reclen)
       Read(interp_unit,rec=1) buff
       Close(interp_unit)
       olddata = Dble(buff)
       Deallocate(buff)
    Else
       !Read in bulk data
       Open(interp_unit, file=Trim(iter_filename), form='unformatted', access='direct', status='unknown',recl=reclen)
       Read(interp_unit,rec=1) olddata
       Close(interp_unit)
    EndIf

    If (cartesian) Then
       !Call Interp3d(phi,oldtheta,oldr,newx,newy,newz)
		Call Interp3d(phi,theta,oldr,newx,newy,newz)
  ...
  Subroutine Interp3d(ox, oy, oz, nx, ny, nz)
    ...
    Integer :: i1, i2, i3, ii, ij, ik, chunk !loop variables
       ...
       Do i2=1,nny
          ...
          Do i1=1,nnx
             ...
             Else
                ...
                ii = 1
                Do While ((ii .lt. ony) .and. (tc .gt. oy(ii))) 
                   ii = ii + 1
                EndDo
                iy(2) = Min(Max(ii-1,1),ony)
                iy(3) = Min(iy(2)+1,ony)
